...
Lưu ý: các view về bạn bè (friends_list, add_friend, unfriend,
friends_information, bio, bio_information_change, logout) chưa được
gắn vào urls.py nên phải gọi thẳng hàm view bằng RequestFactory.
"""
...
from apps.test_helpers import (
    THU_MUC_ANH_TAM, TestCoDuLieu, anh_mau, ket_ban, tao_nguoi_dung,
)
...
class TrangCheckTest(TestCoDuLieu):
    ...
    def test_chua_co_bio_thi_thay_form_tao_avatar(self):
        """Ai chưa có Bio mở trang check thì thấy form để nộp avatar."""
        user = User.objects.create_user('moi', 'moi@example.com', 'mat-khau-manh-123')
        self.client.force_login(user)
        response = self.client.get(reverse('check'))
        self.assertEqual(response.status_code, 200)

    def test_nop_avatar_thi_duoc_tao_bio(self):
        """Người chưa có Bio nộp avatar thì được tạo Bio rồi về trang chủ."""
        user = User.objects.create_user('moi', 'moi@example.com', 'mat-khau-manh-123')
        self.client.force_login(user)
        response = self.client.post(reverse('check'), {'avatar': anh_mau()})
...
class BanBeTest(TestCoDuLieu):
    ...
    def _request_cua(self, user, method='get'):
        """Dựng một request đã đăng nhập sẵn cho user."""
        request = getattr(self.rf, method)('/ban-be')
        request.user = user
        return request

    def test_ket_ban_thanh_cong_ca_hai_chieu(self):
        """An gửi kết bạn với Bình thì hai người thành bạn của nhau."""
        views.add_friend(self._request_cua(self.user_an, 'post'), self.bio_binh.id)
    ...
    def test_ket_ban_voi_nguoi_da_la_ban_thi_bao_loi(self):
        ...
        response = views.add_friend(self._request_cua(self.user_an, 'post'), self.bio_binh.id)
        self.assertGreaterEqual(response.status_code, 400)

    def test_huy_ket_ban_thanh_cong_ca_hai_chieu(self):
        """Đang là bạn mà hủy kết bạn thì cả hai phía đều hết là bạn."""
        ket_ban(self.bio_an, self.bio_binh)
        views.unfriend(self._request_cua(self.user_an, 'post'), self.bio_binh.id)
    ...
    def test_huy_ket_ban_voi_nguoi_la_thi_bao_loi(self):
        response = views.unfriend(self._request_cua(self.user_an, 'post'), self.bio_binh.id)
        self.assertGreaterEqual(response.status_code, 400)

    def test_xem_danh_sach_ban_be(self):
        ket_ban(self.bio_an, self.bio_binh)
        response = views.friends_list(self._request_cua(self.user_an))
        self.assertEqual(response.status_code, 200)

    def test_xem_thong_tin_cua_ban(self):
        ket_ban(self.bio_an, self.bio_binh)
        response = views.friends_information(self._request_cua(self.user_an), self.bio_binh.id)
        self.assertEqual(response.status_code, 200)

    def test_xem_thong_tin_nguoi_khong_ton_tai_thi_bao_loi(self):
        response = views.friends_information(self._request_cua(self.user_an), 999999)
        self.assertGreaterEqual(response.status_code, 400)
...
class TrangCaNhanTest(TestCoDuLieu):
    ...
    def test_doi_thong_tin_ca_nhan(self):
        ...
        request.user = self.user
        views.bio_information_change(request)
